...
func (w *Watcher) reloadClients(rescanAuth bool, affectedOAuthProviders []string, forceAuthRefresh bool) {
	...
	geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, toCodexAPIKeyCount, openAICompatCount := BuildAPIKeyClients(cfg)
	totalAPIKeyClients := geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + toCodexAPIKeyCount + openAICompatCount
	...
	totalNewClients := authFileCount + geminiAPIKeyCount + vertexCompatAPIKeyCount + claudeAPIKeyCount + codexAPIKeyCount + toCodexAPIKeyCount + openAICompatCount
	...
	log.Infof("full client load complete - %d clients (%d auth files + %d Gemini API keys + %d Vertex API keys + %d Claude API keys + %d Codex keys + %d ToCodex keys + %d OpenAI-compat)",
		totalNewClients,
		authFileCount,
		geminiAPIKeyCount,
...
func BuildAPIKeyClients(cfg *config.Config) (int, int, int, int, int, int) {
	...
	if len(cfg.ToCodexKey) > 0 {
		for _, toCodexConfig := range cfg.ToCodexKey {
			toCodexAPIKeyCount += toCodexConfig.ToCodexAPIKeyCount()
		}
	}
	...
	return geminiAPIKeyCount, vertexCompatAPIKeyCount, claudeAPIKeyCount, codexAPIKeyCount, toCodexAPIKeyCount, openAICompatCount
}
